...
export default function HomePage() {
  return <>
    <section className="hero home-hero"><div className="wrap hero-grid"><div className="hero-copy"><div className="trust-chip"><span className="trust-dot" /> 실제 상점·공식 공지 기준</div><p className="eyebrow">MONGDDANG DIA · LINM</p><h1>린M 다이아·충전·상점 정보를<br/><em>실제 자료 기준으로</em></h1><p className="lead">몽땅 다이아는 린M: 잊혀진 유산의 실제 게임 내 상점 스크린샷과 공식 라운지 공지를 바탕으로 다이아 가격, 재화, 아이템, 할인, 이벤트와 업데이트 정보를 정리합니다.</p><div className="hero-actions"><a className="button primary" href={siteConfig.phoneHref}>할인충전문의: {siteConfig.phone}</a><a className="button ghost" href={siteConfig.kakaoUrl} target="_blank" rel="noreferrer">카톡 문의: {siteConfig.kakaoId}</a></div><div className="hero-trust-row"><span>실제 충전 화면</span><span>공식 라운지 공지</span><span>2026-09-02 자료 기준</span></div></div><div className="hero-visual"><div className="visual-frame"><div className="visual-badge">LINM</div><div className="diamond-mark">◇</div><strong>몽땅 다이아</strong><span>linm-diamond-charge-price.webp</span></div><div className="floating-card"><small>QUICK CONTACT</small><strong>{siteConfig.phone}</strong><span>카카오톡 {siteConfig.kakaoId}</span></div></div></div></section>
